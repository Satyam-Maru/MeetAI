...
router.post("/signup", async (req, res) => {
  try {
    ...
    if (existingUser) {
      return res
        .status(400)
        .json({ error: "Account already exists, try signing in." });
    }
...
router.post("/verify", async (req, res) => {
  try {
    ...
    const assignInitialPFP = (username) =>
      `${process.env.CLOUDINARY_URL}/${username[0].toUpperCase()}.png`;
...
router.post("/reset-password", async (req, res) => {
  try {
    ...
    if (!storedCode) {
      return res
        .status(400)
        .json({ error: "Reset code has expired. Please try again." });
    }
  ...
  try {
    ...
    if (cachedUserData) {
      ...
      if (userPayload.isGoogleUser) {
        return res.status(400).json({
          error:
            'This account is registered with Google. Please use "Continue with Google".',
        });
      }

      // Check if password if the user is not google user
